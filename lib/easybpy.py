...
def is_object_in_view_layer(obj):
    """Check if an object is available in the current view layer"""
    try:
...
def ensure_object_in_view_layer(obj):
    """Ensure an object is accessible in the current view layer"""
    if is_object_in_view_layer(obj):
        return True
    
    # Try to make the object accessible by linking its collections
    for collection in obj.users_collection:
        try:
            # Check if collection is linked to the scene
            if collection not in bpy.context.scene.collection.children_recursive:
                bpy.context.scene.collection.children.link(collection)
                print(f"Linked collection '{collection.name}' to make object '{obj.name}' accessible")
                return True
        except RuntimeError:
            # Collection might already be linked, continue
            continue
    
    return is_object_in_view_layer(obj)
...
def select_object(ref, make_active=True):
    """Select an object, ensuring it's in the current view layer for Blender 4.5+ compatibility"""
    objref = get_object(ref)
    
    # Ensure object is accessible in view layer
    if not ensure_object_in_view_layer(objref):
        print(f"Warning: Object '{objref.name}' could not be made accessible in the current view layer")
        return False
    
    # Now try to select the object
    try:
...
def select_objects(ref):
    """Select multiple objects, ensuring they're in the current view layer"""
    objref = get_objects(ref)
    for o in objref:
        # Use the updated select_object function which handles view layer issues
        select_object(o, make_active=False)
...
def select_only(ref = None):
    """Select only the specified object, deselecting all others first"""
    if ref is None:
        return
    ...
    # Use the improved select_object function
    success = select_object(objref, True)
    if not success:
        print(f"Warning: Could not select object '{objref.name}' exclusively")
...
def deselect_all_objects():
    ...
    # Use context selected objects to avoid view layer issues
    for ob in bpy.context.selected_objects:
        try:
            ob.select_set(False)
        except RuntimeError:
            # Skip objects that can't be deselected
            continue
