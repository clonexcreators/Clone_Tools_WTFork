...
updater.addon = "blender_clone_tools"
...
class AddonUpdaterCheckNow(bpy.types.Operator):
    ...
    def execute(self, context):
        global manual_check_in_progress
        if updater.invalid_updater:
            return {'CANCELLED'}
        ...
        # Avoid Blender API usage from background thread callbacks.
        updater.check_for_update_now(callback=None)
        manual_check_in_progress = True
        _start_async_update_poll()
...
background_check_in_progress = False
...
def background_update_callback(update_ready):
    """Handle background update result on main thread."""
    global ran_auto_check_install_popup
...
def _async_update_check_poll_timer():
    """Poll async updater state from Blender main thread."""
    global background_check_in_progress
    global manual_check_in_progress

    if updater.async_checking:
        return 0.5

    if manual_check_in_progress:
        manual_check_in_progress = False
        ui_refresh(updater.update_ready)

    if background_check_in_progress:
        background_check_in_progress = False
        background_update_callback(bool(updater.update_ready))

    return None


def _start_async_update_poll():
    if not bpy.app.timers.is_registered(_async_update_check_poll_timer):
        bpy.app.timers.register(_async_update_check_poll_timer, first_interval=0.5)
...
def check_for_update_background():
    ...
    *Could* be called on register, but would be bad practice as the bare
    ...
    # Avoid Blender API usage from background thread callbacks.
    updater.check_for_update_async(callback=None)
    background_check_in_progress = True
    _start_async_update_poll()
...
def update_notice_box_ui(self, context):
    ...
    if not updater.manual_only:
        ...
        # col.operator("wm.url_open", text="Support Clone.Tools!", icon="FUND").url = updater.website
...
def register(bl_info):
    ...
    updater.user = "CloneTools"

    # Choose your own repository, must match git name for GitHUb and Bitbucket,
    # for GitLab use project ID (numbers only).
    updater.repo = "blender-clone-tools"

    # updater.addon = # define at top of module, MUST be done first

    # Website for manual addon download, optional but recommended to set.
    updater.website = "https://rtfktbeb.gumroad.com/l/clonetools"
